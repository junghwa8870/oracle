...
IS -- 선언부의 끝을 나타냄
    v_cnt NUMBER := 0; --  v_cnt는 숫자 값을 저장하는 변수이며, 초기값으로 0을 가집니다.
BEGIN -- 본문
...
-SET은 SQL*Plus와 같은 SQL 실행 환경에서 사용되는 명령어입니다. 

1. 환경 설정 변경: SQL*Plus에서는 SET을 사용하여 다양한 실행 환경 설정을 변경할 수 있습니다. 
                예를 들어, SET SERVEROUTPUT ON은 DBMS 출력을 활성화하고, 
                SET AUTOTRACE ON은 실행 계획과 관련된 정보를 출력합니다.

2. 결과 표시 형식 변경: SET을 사용하여 결과의 표시 형식을 변경할 수 있습니다. 
                예를 들어, SET LINESIZE를 사용하여 출력 라인의 길이를 조정하거나, SET NUMWIDTH를 사용하여 숫자 값의 표시 너비를 조정할 수 있습니다.

3. 사용자 편의 기능 활성화: SET은 SQL*Plus에서 사용자 편의 기능을 활성화하는 데에도 사용됩니다. 
                예를 들어, SET FEEDBACK을 사용하여 실행 결과에 대한 피드백을 활성화하거나 비활성화할 수 있습니다.

4. 쿼리 결과 제한: SET ROWLIMIT을 사용하여 쿼리 결과의 행 수를 제한할 수 있습니다.
...
BEGIN -- 본문
    ...
    -- OUT 매개변수에 조회 결과를 할당. -> 우리가 알고 있는 return 개념과 같다.
...
BEGIN
    ...
    my_new_job_proc('CEO1', 'test_job2', 30000, 80000, msg);
    dbms_output.put_line(msg);
END;
