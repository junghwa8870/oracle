...
UPDATE dept2
SET dept_no = 20
WHERE dept_no = 10; -- 실패(주요 키 제약조건 위반)

UPDATE dept2
SET dept_bonus = 900
WHERE dept_no = 10; -- 실패(CHECK 키 제약조건 위반)
...
FOREIGN KEY(loka) REFERENCES locations(location_id);

-- check 추가
ALTER TABLE dept ADD CONSTRAINT dept2_bonus_ck CHECK(dept_bonus > 10000);

-- unique 추가
ALTER TABLE dept ADD CONSTRAINT dept2_deptname_uk UNIQUE(dept_name);
